...
class RepType(Base):
    ...
    # goal_id — NOT NULL FK to goals.id. Same pattern as Rep.goal_id.
    goal_id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("goals.id"), nullable=False, index=True)

    # name — Text, NOT NULL. Examples: "Outbound rep", "Build rep".
    ...
    # duration_minutes — Integer, NOT NULL. No default (force user to specify
    # 15–90 minute range explicitly — could add a CheckConstraint, but skip for v1).
    duration_minutes: Mapped[int] = mapped_column(Integer, nullable=False)

    # daily_floor — Integer, nullable. Minimum reps per day. NULL means
    ...
    # weekly_target — Integer, nullable. Target reps per week.
    weekly_target: Mapped[int | None] = mapped_column(Integer, nullable=True)

    # is_first_rep — Boolean, NOT NULL, default=False.
    ...
    # emoji — String(8), nullable. Used in dashboard rendering.
    emoji: Mapped[str | None] = mapped_column(String(8), nullable=True)

    # display_order — Integer, NOT NULL, default=0. Controls UI ordering.
    display_order: Mapped[int] = mapped_column(Integer, nullable=False, default=0)

    # status — RepTypeStatus enum, NOT NULL, default=RepTypeStatus.active.
    ...
    # goal relationship — back_populates="rep_types".
    goal: Mapped["Goal"] = relationship(back_populates="rep_types")

    # reps relationship — back_populates="rep_type".
