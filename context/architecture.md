...
| Technical 5 — calendar failures degrade | `create_event` returns `None` on failure (`google_calendar.py:110`), and callers assign it to `rep.calendar_event_id` without checking | **Bug, and it has already happened.** The invariant holds — nothing raises — but the rep is then permanently unsynced with no record and no retry. Confirmed 2026-09-07: the OAuth refresh token returns `invalid_grant`, the last rep to receive a calendar event was 2026-06-04, and 136 reps created since then have none. Sync has been dead for three months and the system never said so. The failure log does not even name the event id (`google_calendar.py:84,108,129`), so there is no way to tell which reps failed. This is the "calendar sync fails silently" V1-failure condition, live. |
